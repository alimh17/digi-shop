...
const Nintendo = ({ Enter, Leave, consoleGame }) => {
  return (
    <div
    ...
    >
      <span className="max-w-md w-32 ">نینتندو</span>
      <BsChevronLeft />
      <div
        className=" hidden absolute md:w-6/6 lg:w-4/6 xl:w-3/6  right-52 top-0 h-full border-2 border-gray-300 rounded-lg"
        style={consoleGame[2] ? { display: "flex" } : { display: "none" }}
      >
        <div className="flex flex-col gap-2  w-full overflow-auto ">
          {consoleGames[0].nintendo.map((item, index) => (
            <span
              key={index}
