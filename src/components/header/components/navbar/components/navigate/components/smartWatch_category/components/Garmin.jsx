...
const Garmin = ({ Enter, Leave, smartWatch }) => {
  return (
    <div
    ...
    >
      <span className="max-w-md w-32 ">گارمین</span>
      <BsChevronLeft />
      <div
        className=" hidden absolute w-3/6 left-1/3 top-0 h-full border-2 border-gray-300 rounded-lg"
        style={smartWatch[5] ? { display: "flex" } : { display: "none" }}
      >
        <div className="flex flex-col gap-2  w-full overflow-auto">
          {smartWatchs[0].garmin.map((item, index) => (
            <span
              key={index}
